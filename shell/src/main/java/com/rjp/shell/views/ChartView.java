...
public class ChartView extends View {
     ...
     * @param cellGroups
     */
    public void addChart(List<Cell> topCells, List<CellGroup> cellGroups) {
        this.topCells = topCells;
        this.cellGroups = cellGroups;
        int columnCount = cellGroups.get(0).getCells().size();
     ...
     */
    public void addAnotherChart(List<Cell> blueTopCells, List<CellGroup> bluecellsGroup) {
        //顶部栏也加一个空的cell
        topCells.add(new Cell());
        int size1 = blueTopCells.size();
        for (int i = 0; i < size1; i++) {
            topCells.add(blueTopCells.get(i));
        }
